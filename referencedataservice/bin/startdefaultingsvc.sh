...
LOGGING_CONFIG="-Dlog4j.configurationFile=${CONFIG_LOC}/defaultingsvclog4j2config.xml"

if [ "$1" != "DEBUG" ]; then
    ./start.sh com.droitfintech.defaultingservice.DefaultingDataService defaultingsvc $LOGGING_CONFIG
else
    ./start.sh com.droitfintech.defaultingservice.DefaultingDataService defaultingsvc $LOGGING_CONFIG DEBUG 7777
fi

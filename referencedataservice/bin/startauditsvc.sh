#!/usr/bin/env bash
# ------------------------------------------------------------------------------------
# startauditsvc.sh - Start/Stop Script for the Audit Service
...
LOGGING_CONFIG="-Dlog4j.configurationFile=${CONFIG_LOC}/auditsvclog4j2config.xml"


if [ "$1" != "DEBUG" ]; then
   ./start.sh com.droitfintech.auditservice.AuditReferenceDataService auditsvc $LOGGING_CONFIG
else
   ./start.sh com.droitfintech.auditservice.AuditReferenceDataService auditsvc $LOGGING_CONFIG DEBUG 9797
fi
